...
| Canonical Resource Identity attribute references | [resource-identity.md#canonical-attribute-references](../../specification/resource-identity.md#canonical-attribute-references) | `CanonicalResourceIdentityAttribute` | [common/identifiers.ts](../../packages/core/src/model/common/identifiers.ts) | Yes | Implemented | Closed union used by Specification v1.1 naming semantics |
...
| Naming Casing | [convention-pack.md#casing](../../specification/convention-pack.md#casing) | `NamingCasing` | [conventions/naming-casing.ts](../../packages/core/src/model/conventions/naming-casing.ts) | Yes | Implemented | Closed `preserve` / `lower` / `upper` vocabulary used by naming projections |
| Resource Definition | [resource-definition.md](../../specification/resource-definition.md) | `ResourceDefinition`, `ResourceIdentityConstraints`, `ResourceRenderingConstraints`, `ResourceNameCharacterClass`, `ResourceNameCharacterSet`, `PlacementConstraint` | [definitions/resource-definition.ts](../../packages/core/src/model/definitions/resource-definition.ts), [definitions/resource-name-character-class.ts](../../packages/core/src/model/definitions/resource-name-character-class.ts), [definitions/resource-name-character-set.ts](../../packages/core/src/model/definitions/resource-name-character-set.ts), [definitions/placement-constraint.ts](../../packages/core/src/model/definitions/placement-constraint.ts) | Yes | Partially implemented | [Specification v1.2](../../specification/README.md#specification-v12-executable-resource-constraints) is implemented for the closed length, character, boundary, reserved-pattern, and structured placement fields; the Specification's own ["Out of scope"](../../specification/resource-definition.md#out-of-scope-for-this-document) section still defers a concrete JSON Schema and several non-executable concepts remain deferred |
| Convention Pack | [convention-pack.md](../../specification/convention-pack.md) | `ConventionPack`, `ConventionPackIdentityDefaults`, `ConventionPackOverridePolicy` | [conventions/convention-pack.ts](../../packages/core/src/model/conventions/convention-pack.ts) | Yes | Partially implemented | Normalization rules and metadata projection mappings are intentionally not modeled; the Specification's own ["Out of scope"](../../specification/convention-pack.md#out-of-scope) section defers their concrete syntax. Specification v1.1 formalized `separator`, `casing`, `abbreviations`, and `naming_component_order`; the TypeScript contract now reflects those naming fields, while normalization and metadata projection remain deferred. |
...
| Convention Validation | [convention-result.md#conceptual-contents](../../specification/convention-result.md#conceptual-contents) | `ConventionValidation`, `ConventionValidationFailure`, `ConventionValidationFailureCode`, `ConventionWarning` | [results/convention-result.ts](../../packages/core/src/model/results/convention-result.ts) | Yes | Implemented | A `valid` flag plus optional failures/warnings; v1.2 executable constraint failures carry the closed optional `code` vocabulary while legacy failures remain uncoded |
