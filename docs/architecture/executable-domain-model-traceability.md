...
| Convention Pack | [convention-pack.md](../../specification/convention-pack.md) | `ConventionPack`, `ConventionPackIdentityDefaults`, `ConventionPackOverridePolicy` | [conventions/convention-pack.ts](../../packages/core/src/model/conventions/convention-pack.ts) | Yes | Partially implemented | Normalization rules and metadata projection mappings are intentionally not modeled; the Specification's own ["Out of scope"](../../specification/convention-pack.md#out-of-scope) section defers their concrete syntax. Specification v1.1 additionally formalized `separator`, `casing`, and reshaped `abbreviations` semantics (see [convention-pack.md#naming-projections](../../specification/convention-pack.md#naming-projections)); `ConventionPack`'s TypeScript contract does not yet reflect these — Specification defined, executable model update pending (see `IMPLEMENTATION.md`'s planned 2.6.2 increment). |
