...
| Allowed characters (descriptive) | [resource-definition.md#character-constraints](../../specification/resource-definition.md#character-constraints) | `ResourceRenderingConstraints.allowed_characters_description: string` | — | Modelled but not executable | Free text, retained only as human-readable documentation | Specification |
| Allowed characters (structured, `character_constraints`) | [resource-definition.md#character-constraints](../../specification/resource-definition.md#character-constraints) | `ResourceNameCharacterSet` | `validateRenderingConstraints` | Executable | Closed ASCII character classes plus single-code-point literals; deliberately not a regex | Reference Evaluator |
| Start/end constraints (`starts_with`/`ends_with`) | [resource-definition.md#startend-constraints](../../specification/resource-definition.md#startend-constraints) | `ResourceNameCharacterSet` | `validateRenderingConstraints` | Executable | First/last code point checks in the normative `starts_with` then `ends_with` order | Reference Evaluator |
| Reserved prefixes/suffixes (`forbidden_prefixes`/`forbidden_suffixes`) | [resource-definition.md#reserved-prefixes-and-suffixes](../../specification/resource-definition.md#reserved-prefixes-and-suffixes) | `ReadonlyArray<string>` | `validateRenderingConstraints` | Executable | Exact-match, case-sensitive checks; every matching entry is reported in declaration order | Reference Evaluator |
...
| Minimum-name length (`min_length`) | [resource-definition.md#minimum-length](../../specification/resource-definition.md#minimum-length) | `ResourceRenderingConstraints.min_length: number` | `validateRenderingConstraints` | Executable | Shares the declared closed `code_points` / `utf8_bytes` unit with `max_length` and is evaluated first | Reference Evaluator |
...
| Technical constraints (`allowed_characters` / `character_constraints`) | [resource-definition.md#character-constraints](../../specification/resource-definition.md#character-constraints) | `allowed_characters_description` plus `ResourceNameCharacterSet` | `validateRenderingConstraints` | Executable for structured constraints; descriptive field remains non-executable | Free text is not interpreted; the structured character set is evaluated | Reference Evaluator |
| Technical constraints (`min_length`) | [resource-definition.md#minimum-length](../../specification/resource-definition.md#minimum-length) | `ResourceRenderingConstraints.min_length` | `validateRenderingConstraints` | Executable | See the Naming table's `min_length` row above | Reference Evaluator |
| Technical constraints (`starts_with`/`ends_with`, `forbidden_prefixes`/`forbidden_suffixes`) | [resource-definition.md#startend-constraints](../../specification/resource-definition.md#startend-constraints), [resource-definition.md#reserved-prefixes-and-suffixes](../../specification/resource-definition.md#reserved-prefixes-and-suffixes) | Structured fields on `ResourceRenderingConstraints` | `validateRenderingConstraints` | Executable | See the corresponding Naming table rows above | Reference Evaluator |
...
| Placement Constraints | [resource-definition.md#structured-placement-constraints-specification-v12](../../specification/resource-definition.md#structured-placement-constraints-specification-v12) | `ReadonlyArray<PlacementConstraint>` | `validatePlacementConstraints` | Executable within the stated limits | Closed `equals`/`present`/`absent` rules evaluate canonical Resource Identity subjects in declaration order; statement-only entries and the ACM/CloudFront relationship remain non-executable | Reference Evaluator |
...
`ResourceDefinition.placement_constraints?: ReadonlyArray<PlacementConstraint>` now has
the structured v1.2 shape: every entry retains a descriptive `statement` and may add a
machine-readable `rule` using an existing canonical Resource Identity subject and the
closed `equals`/`present`/`absent` operator vocabulary. The Reference Evaluator checks
rule entries in declaration order; a false condition skips its rule without producing a
failure, and statement-only entries remain descriptive. The ACM Certificate + CloudFront
→ `us-east-1` example remains statement-only because no canonical attribute represents
that cross-resource relationship. Classified **Executable within the Specification's
stated limits**.
