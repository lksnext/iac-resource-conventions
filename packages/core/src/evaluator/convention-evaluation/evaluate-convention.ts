...
function explain(
  ...
  const renderingConstraintSummary =
    renderingConstraintFailures.length === 0
      ? ""
      : ` ${renderingConstraintFailures.map((failure) => failure.message).join(". ")}.`;

  const placementConstraintSummary =
    placementConstraintFailures.length === 0
      ? ""
 ...
 *
 * **Implemented rule: executable Resource Definition constraints (Specification
 * v1.2).** `validateRenderingConstraints` and `validatePlacementConstraints` (see
 * `./resource-constraints/index.js`) implement every constraint family Specification
 * v1.2 defines precisely enough to execute: `min_length`, `max_length`,
 * `character_constraints`, `starts_with`, `ends_with`, `forbidden_prefixes`,
 * `forbidden_suffixes`, and executable Placement Constraint `rule`s, in the
 * Specification's own normative deterministic order (see
 ...
 * - normalization (`ResourceRenderingConstraints.normalization` is free text, not a
...
export function evaluateConvention(input: ConventionEvaluationInput): ConventionResult {
  ...
  const placementConstraintFailures = validatePlacementConstraints(
