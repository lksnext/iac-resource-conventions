...
- **`NOTICE`** attributes copyright for this project itself (per Apache License 2.0 section 4(d));
  it is not a dependency inventory, and is not modified when a dependency changes.
...
[license-checker-rseidelsohn](https://github.com/RSeidelsohn/license-checker-rseidelsohn), wrapped
by [`scripts/check-licenses.mjs`](scripts/check-licenses.mjs), inventories every installed
dependency's SPDX license expression against this repository's explicit allowlist:

- `npm run licenses:check` — checks every dependency (production and development); fails with a
  non-zero exit code and a list of violations if any license is missing, unrecognized, or not on
  the allowlist.
- `npm run licenses:production` — the same check, scoped to production dependencies only.
- `npm run licenses:report` — prints the full license inventory for manual review; never fails,
  never writes a generated file.

See
[`IMPLEMENTATION.md#dependency-license-validation`](IMPLEMENTATION.md#dependency-license-validation)
for the full, current allowlist and the step-by-step process for proposing an exception. In
short: **a passing automated check is not legal advice.** Contributors must still verify that a
new dependency's use and distribution model are compatible with this project, and allowlist
exceptions must never be added silently — they require the review process documented in
`IMPLEMENTATION.md` and explicit maintainer approval.

`licenses:check` is fully offline and deterministic, but it is intentionally **not** part of
`npm run validate` or `pre-commit`/lint-staged: some dependencies install different optional,
platform-specific packages depending on the operating system (for example Biome's per-OS
`@biomejs/cli-*` binaries), so the exact set of licensed packages is not always identical across
every environment `validate` runs in. It instead runs once, on Linux only, in its own CI job —
see [Continuous Integration](#continuous-integration) below — and can be run locally at any time
with `npm run licenses:check` / `npm run licenses:production`.
